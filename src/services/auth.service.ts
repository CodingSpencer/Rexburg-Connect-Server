...
export class AuthService {
    public static async login(credentials: LoginDto): Promise<AuthResponse> {
        const { profileName, password } = credentials;

        // Select the password field since hidden and execute the query
        const user = await User.findOne({ profileName }).select('+password').exec();
        if (!user) {
        ...
        const token = jwt.sign(
            { userId: user._id, profileName: user.profileName, email: user.email },
            jwtSecret,
